...
def expand_shear_jobs(growth_ready):
    jobs = []
    for params in growth_ready:
        jobs.append(
            {
                **params,
                "source_tag": "DPHI",
...
def main():
    ...
    shear_jobs = expand_shear_jobs(growth_ready)
    queued_by_tag = Counter(job["source_tag"] for job in shear_jobs)
    completed, skipped, failed = execute_param_jobs(
        ...
        lambda params: run_shear_stage(
            ...
            required_input=params["required_input"],
        ),
        args.n_cpus,
        lambda params, result: (
            f"Failed {params['source_tag']} shear job: "
            f"lx={params['lx']} p0={params['p0']} dphi={params['dphi']} seed={params['seed']} "
            f"reason={result['reason']}"
        ),
    )
    print(f"Growth-complete parameter tuples: {len(growth_ready)}")
